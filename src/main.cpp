...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OrangeCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "OrangeCoin Signed Message:\n";
...
{
    // OrangeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 2 * 60 * 60; // OrangeCoin: every 2 hours
static const int64 nTargetSpacing = 60; // OrangeCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // OrangeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // OrangeCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today is March 8th 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OrangeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OrangeCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OrangeCoinMiner(CWallet *pwallet)
{
    printf("OrangeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("OrangeCoin-miner");
    ...
    try { loop {
        ...
        printf("Running OrangeCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OrangeCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OrangeCoinMiner, pwallet));
}
